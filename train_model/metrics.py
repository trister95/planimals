...
def compute_metrics_without_O(p, label_lst):
    ...
    true_predictions = [
        [label_lst[p] for (p, l) in zip(prediction, label) if l != -100 and p != O_index and l != O_index]
        for prediction, label in zip(predictions, labels)
    ]
    true_labels = [
        [label_lst[l] for (p, l) in zip(prediction, label) if l != -100 and p != O_index and l != O_index]
        for prediction, label in zip(predictions, labels)
    ]
