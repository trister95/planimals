...
from utils import (
    ...
    clean_whitespace,
    find_language_in_ucto_tokenized_sentence,
    has_letter,
)
...
def txt_to_folia(input_dir, output_dir):
    ...
    for f in tqdm.tqdm(
        list_extension_files(input_dir, "txt"), desc="Looping through text files"
    ):
        try:
...
def select_random_files(path_to_folder, file_extension, num_files):
    ...
    try:
        ...
        if num_files > len(file_paths):
            print(
                f"Warning: Requested {num_files} files, but only {len(file_paths)} files are available."
            )
...
def folia_to_df(folia_path):
    ...
    try:
        ...
        for s in doc.sentences():
            ...
            if (
                has_letter(sentence_text)
                and find_language_in_ucto_tokenized_sentence(sentence_text.split())
                == "nl"
            ):
...
def join_meta_csv(df, csv_path, left_on="text_id", right_on="ti_id", sep=","):
    ...
    """
    metadata = (
        pd.read_csv(csv_path, sep=sep)
        .drop_duplicates(subset="ti_id")
...
def process_chunk(dataframes, subset = None):
    """
    Concatenates dataframes, drop duplicates from a subset, 
    and shuffles the resulting dataframe in random order.
    """
    combined = pd.concat(dataframes)
    unique = combined.drop_duplicates(subset = subset)
